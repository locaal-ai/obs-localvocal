...
void TokenBufferThread::initialize(struct transcription_filter_data* gf_, std::function<void(const std::string &)> callback_, size_t maxSize_,
...
void TokenBufferThread::monitor()
{
    ...
    while (this->initialized && !this->stop) {
        ...
        for (const auto &token : emitted) {
            const char *token_str = whisper_token_to_str(this->gf->whisper_context, token.id);
            output += token_str;
        }
